...
Servlet handles request to login, logout, add new users, edit/save existing systems		
************************************************************************************************/
public class UserServlet extends HttpServlet {
		...
		String userMessage	= null;
			...
			switch (validActions.valueOf(action))
			{
				...
				case init:
					...
					request.setAttribute("users", cmsQueryUsers.qryUsers(null));//returns all existing users of the system
					request.setAttribute("roles", cmsQueryUsers.qryRoles());// returns all system roles
	...
	//Encryption code from Jan 7 '13 at 18:28, Adrian Stamin (http://stackoverflow.com/questions/6592010/encrypt-and-decrypt-a-password-in-java)
	private String encrypt(String password) throws NoSuchAlgorithmException 
	{
		
		MessageDigest md = MessageDigest.getInstance("MD5");//uses the Message-digest algorithm
		md.reset();
        byte[] passBytes = password.getBytes();
        byte[] digested = md.digest(passBytes);
        StringBuffer buff = new StringBuffer();
        for(int i=0;i<digested.length;i++){
        	buff.append(Integer.toHexString(0xff & digested[i]));
        }
